...
import { 
  Eye, EyeOff, Lock, Mail, User, Building, CheckCircle2, 
  Map, Phone, Briefcase, Globe, MapPin, Hash, ChevronLeft 
} from "lucide-react";
...
export default function RegisterPage() {
  ...
  useEffect(() => {
    const requiredFields = [
      ...
      'street', 'number', 'adminPhone', 'companyPhone', 'password'
    ];
  ...
  const validateStep1 = () => {
    ...
    if (!formData.cnpj || !/^\d{14}$/.test(formData.cnpj)) newErrors.cnpj = "CNPJ inválido (14 dígitos)";
    if (!formData.corporateEmail || !/\S+@\S+\.\S+/.test(formData.corporateEmail))
      newErrors.corporateEmail = "Email corporativo inválido";
    if (!formData.adminFirstName) newErrors.adminFirstName = "Nome do administrador é obrigatório";
    if (!formData.adminLastName) newErrors.adminLastName = "Sobrenome do administrador é obrigatório";
    if (!formData.industry) newErrors.industry = "Ramo de atuação é obrigatório";
  ...
  const validateStep2 = () => {
    ...
    if (!formData.adminPhone) newErrors.adminPhone = "Telefone do administrador é obrigatório";
  ...
  const handleFinalSubmit = async (withManager: boolean) => {
    setLoading(true);
    setFormData(prev => ({ ...prev, hasPurchaseManager: withManager }));
    
    try {
      const res = await fetch("/api/auth/register", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({...formData, hasPurchaseManager: withManager}),
      });
          ...
          >
            <div className="space-y-4 my-6">
              <div className="space-y-1">
                ...
                {errors.companyName && <p className="text-rose-400 text-xs">{errors.companyName}</p>}
              </div>
              
              <div className="space-y-1">
                <Label htmlFor="cnpj" className="text-white/80 flex items-center gap-2">
                ...
                />
                {errors.cnpj && <p className="text-rose-400 text-xs">{errors.cnpj}</p>}
              </div>
              
              <div className="space-y-1">
                <Label htmlFor="corporateEmail" className="text-white/80 flex items-center gap-2">
                ...
                />
                {errors.corporateEmail && <p className="text-rose-400 text-xs">{errors.corporateEmail}</p>}
              </div>
              
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <Label htmlFor="adminFirstName" className="text-white/80 flex items-center gap-2">
                  ...
                  />
                  {errors.adminFirstName && <p className="text-rose-400 text-xs">{errors.adminFirstName}</p>}
                </div>
                
                <div className="space-y-1">
                  <Label htmlFor="adminLastName" className="text-white/80 flex items-center gap-2">
                    Sobrenome
                  </Label>
                  <Input 
                    id="adminLastName" 
                  ...
                  />
                  {errors.adminLastName && <p className="text-rose-400 text-xs">{errors.adminLastName}</p>}
                </div>
              </div>
              
              <div className="space-y-1">
                <Label htmlFor="industry" className="text-white/80 flex items-center gap-2">
                  <Briefcase className="h-4 w-4 text-indigo-400" /> Ramo de Atuação
                </Label>
                <Input 
                  id="industry" 
                  name="industry" 
                ...
                />
                {errors.industry && <p className="text-rose-400 text-xs">{errors.industry}</p>}
              </div>
            </div>
        ...
        return (
          ...
          >
            <div className="space-y-4 my-6">
              ...
              <div className="space-y-1">
                <Label htmlFor="adminPhone" className="text-white/80 flex items-center gap-2">
                  <Phone className="h-4 w-4 text-indigo-400" /> Telefone do Administrador
                </Label>
                <Input 
                  id="adminPhone" 
                  name="adminPhone" 
                ...
                />
                {errors.adminPhone && <p className="text-rose-400 text-xs">{errors.adminPhone}</p>}
              </div>
              
              <div className="space-y-1">
                <Label htmlFor="companyPhone" className="text-white/80 flex items-center gap-2">
                  <Phone className="h-4 w-4 text-indigo-400" /> Telefone da Empresa
                </Label>
                <Input 
                  id="companyPhone" 
                  name="companyPhone" 
                ...
                />
                {errors.companyPhone && <p className="text-rose-400 text-xs">{errors.companyPhone}</p>}
              </div>
        ...
        return (
          ...
          >
            <div className="text-center mb-6">
              ...
              <p className="text-white/70 mt-2">Escolha o tipo de conta para sua empresa</p>
            </div>
            ...
            <div className="grid grid-cols-1 gap-4">
              ...
              >
                ...
                <p className="text-white/60 text-sm ml-7">Acesso imediato à plataforma sem gerente de compras dedicado.</p>
              </motion.div>
              ...
              >
                ...
                <p className="text-white/70 text-sm ml-7">
                  Inclui um gerente pessoal para otimizar seus processos de importação.
                  <span className="block mt-1 text-white/50">Aguarde até 1 dia para aprovação (custos adicionais após aprovação).</span>
                </p>
              </motion.div>
            </div>
      ...
      <div className="relative z-10 w-full max-w-md px-4 py-8">
        <div className="bg-black/30 backdrop-blur-xl border border-white/[0.08] rounded-2xl p-6 md:p-8 shadow-xl">
          <motion.div custom={0} variants={fadeInVariants} initial="hidden" animate="visible" className="text-center mb-6">
            ...
            <h1 className="text-2xl font-bold text-white mb-2">Criar Conta Empresarial</h1>
            <p className="text-white/60">Registre sua empresa para acessar nossa plataforma</p>
